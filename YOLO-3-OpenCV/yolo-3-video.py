...
while True:
    ...
    if len(results) > 0:
        ...
        for i in results.flatten():
            ...
            # Preparing color for current bounding box and converting from numpy array to list
    ...
    # Initializing writer (we do it only once from the very beginning when we get spatial dimensions of the frames)
    if writer is None:
        # Constructing code of the codec to be used in the function VideoWriter
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')

        # Writing current processed frame into the video file
        # writer = cv2.VideoWriter('videos/result-traffic-cars.mp4', fourcc, 30, (frame.shape[1], frame.shape[0]), True)
